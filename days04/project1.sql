...
--
--국내선 운임 인서트 3개
insert into dfare values (1,'비수기','정상','선호','일반석','SEOUL/GIMPO-JEJU',97000,3000,'admin001','주중');
insert into dfare values (2,'비수기','정상','일반','일반석','SEOUL/GIMPO-JEJU',115000,3000,'admin002','주말');
insert into dfare values (3,'성수기','정상','일반','일반석','SEOUL/GIMPO-JEJU',132000,3000,'admin005','주중');
...
-- 회원 세부 정보 
INSERT INTO Userdetail (UserID, family_id) VALUES ('user001','#id_01');
INSERT INTO Userdetail (UserID, family_id) VALUES ('user002','#id_01');
INSERT INTO Userdetail (UserID, family_id) VALUES ('user003','#id_02');
INSERT INTO Userdetail (UserID, family_id) VALUES ('user004','#id_02');
INSERT INTO Userdetail (UserID, family_id) VALUES ('user005','#id_02');
...
and to_char(s.ddate,'yymmdd')=dd and d.peak = CASE
    ...
    and d.discount = '정상' and d.sg=sgrade and substr(d.route,1,instr(d.route,'/')-1)=dap and substr(d.route,instr(d.route,'-')+1)=aap and 
...
begin
    select renum  into vrenum from scplane s where to_char(pddate, 'yymmddhh24mi') = to_char(s.ddate , 'yymmddhh24mi') 
    and substr(s.dairport,instr(s.dairport,'.')+1) = pdairport and  substr(s.aairport,instr(s.aairport,'.')+1) = paairport ;
     select d.fare 정상운임 into vcost from scplane s,airplane a ,payrefund p,seat_num sn , dfare d 
where s.asnum=a.asnum and s.renum=p.renum and sn.asnum=s.asnum and sn.seat_num=p.seatnumber 
and substr(s.dairport,instr(s.dairport,'.')+1)=pdairport and substr(s.aairport,instr(s.aairport,'.')+1)=paairport 
and to_char(s.ddate,'yymmddhh24mi')=to_char(pddate,'yymmddhh24mi') and d.peak = CASE
        WHEN to_char(s.ddate, 'yymmdd') = to_date('240101','yymmdd') THEN '성수기'
        WHEN to_char(s.ddate, 'yymmdd') BETWEEN to_date('240208','yymmdd') AND to_date('240213','yymmdd') THEN '성수기'
        WHEN to_char(s.ddate, 'yymmdd') BETWEEN to_date('240224','yymmdd') AND to_date('240302','yymmdd') THEN '성수기'
        WHEN to_char(s.ddate, 'yymmdd') BETWEEN to_date('240503','yymmdd') AND to_date('240506','yymmdd') THEN '성수기'
    ...
    and d.discount = '정상' and d.sg=pseatnum and substr(d.route,instr(d.route,'/')+1,instr(d.route,'-')-1)=pdairport and substr(d.route,instr(d.route,'-')+1)=paairport and 
      d.timezone = case when to_char( s.ddate , 'hh24mi') >= '1500' then '일반' else '선호' end  and d.wknddy  = case to_char(s.ddate , 'dy') when '월' then '주중' when '화' then '주중' when '수' then '주중' when '목' then '주중' else '주말' end;

    insert into payrefund  values
    (mk_payrefundseq.nextval, '결제', ppmethod , '정상', pseatnum ,sysdate, pnluggage ,vcost, 0, 'user001', vrenum);

 end;

exec mk_payrefund_01('2403200930','ICN','CJU','20A',1,'정상','카드',2);
