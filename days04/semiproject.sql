...
--투표결과
select a.choice1 "항목" , lpad(' ' ,a.c+1,'#') "비율" ,a.c "투표자수", a.c/(sum(a.c) over())*100 "백분율"
from (select i.choice1,v.choice_num,v.survey_num, count(choice_num) c from item i left join vote v on i.survey_num=v.survey_num and i.col_number= v.col_number  and v.choice_num = i.cho_num group by i.choice1,v.choice_num,v.survey_num) a
where a.choice1 = any(select choice1 from item where survey_num=1)
order by a.choice1;
...
SET 
    COL_NUMBER = 5,
    TITLE = 'MANAGER',
    START_DATE = TO_DATE('2019'||'08'||'24'),
    END_DATE = TO_DATE('2019'||'08'||'30'),
    WRITE_DATE =TO_DATE('2019'||'08'||'20')   
WHERE SURVEY_NUM = 1;
--1번 서베이 수정
